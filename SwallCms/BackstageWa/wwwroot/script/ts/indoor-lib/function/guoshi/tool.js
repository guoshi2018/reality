...
function correctAllVerticalFlexOffset() {
    document.querySelectorAll('.vrow,.vrowr').forEach(function (row) {
...
function fixFlexGridColMargin(col) {
    var ele_classname = col.className;
    var par = col.parentElement;
    var ok = false;
    if (par) {
        var par_cptStyle = getComputedStyle(par, null);
        if (par_cptStyle.display == "flex") {
            var par_classname = par.className;
...
function fixMarginY(ele, mg) {
    ...
    var cptStyle = getComputedStyle(ele, null);
    // 必须设置为空值, 以便每次调用 getComputedStyle 得到的值, 都与当前的百分数对应
    // 一句话: 不重置, 则第二次调用会设置错误
    ele.style[mg] = '';
    var orig_value = parseFloat(cptStyle[mg]);
    var fixed_value = orig_value * pH / pW;
    ele.style[mg] = "".concat(fixed_value, "px");
}
 ...
 * 	 源元素的拷贝替换
 */
function cloneWithId(root, tag, ref_id) {
    if (root === void 0) { root = document.documentElement || document.body; }
    if (tag === void 0) { tag = "ele-clone"; }
    if (ref_id === void 0) { ref_id = "ref-id"; }
    // 如果要克隆的元素, 存在子级, 就无法避免平行子级重复 id
    Array.from(root.children).forEach(function (ele) {
        var _a;
        if (ele.tagName.toLowerCase() == tag.toLowerCase()) {
            var source_id = ele.getAttribute(ref_id);
            if (source_id) {
                var source = document.getElementById(source_id);
                if (source) {
                    var source_copy = source.cloneNode(true);
                    var id = (_a = ele.id) === null || _a === void 0 ? void 0 : _a.trim();
